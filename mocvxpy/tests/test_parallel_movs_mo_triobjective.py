...
def test_solve_qp_st_linear_constraints_with_MOVS():
    # Parameters
...
def test_solve_ellipsoidal_pb_with_MOVS():
    # Parameters
    a = 7.0
    x = mocp.Variable(3)
    ...
    assert pb.status == "maxiter_reached"
    assert objective_values.shape == (236, 3)
    assert x.values.shape == (236, 3)
    assert np.all(objective_values[:, 0] == objectives[0].values)
    assert np.all(objective_values[:, 1] == objectives[1].values)
    assert np.all(objective_values[:, 2] == objectives[2].values)
...
def test_solve_power_of_4_pb_st_qp_constraints_with_MOVS():
    x = mocp.Variable(2)
    objectives = [
    ...
    # NB: Stop before with default solver options
    objective_values = pb.solve(
        client=CLIENT,
        max_iter=6,
        vertex_selection_solver_options={"solver": cp.CLARABEL},
    ...
    assert pb.status == "maxiter_reached"
    assert objective_values.shape == (276, 3)
    assert x.values.shape == (276, 2)
    assert np.all(objective_values[:, 0] == objectives[0].values)
    assert np.all(objective_values[:, 1] == objectives[1].values)
    assert np.all(objective_values[:, 2] == objectives[2].values)
    x_values = x.values
