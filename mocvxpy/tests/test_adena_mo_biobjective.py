...
def test_solve_circle_pb_with_ADENA():
    x = mocp.Variable(2)
    ...
    assert objective_values.shape == (179, 2)
    assert x.values.shape == (179, 2)
...
def test_solve_norm1_min_st_qp_constraints():
    ...
    objective_values = pb.solve(solver="ADENA")
    assert pb.status == "optimal"
    assert objective_values.shape == (257, 2)
    assert x.values.shape == (257, 2)
    assert np.all(objective_values[:, 0] == objectives[0].values)
    assert np.all(objective_values[:, 1] == objectives[1].values)
    x_values = x.values
...
def test_solve_qp_with_linear_constraints_with_ADENA():
    x = mocp.Variable(2)
    objectives = [
    ...
    objective_values = pb.solve(solver="ADENA")
    assert pb.status == "optimal"
    assert objective_values.shape == (109, 2)
    assert x.values.shape == (109, 2)
    assert np.all(objective_values[:, 0] == objectives[0].values)
    assert np.all(objective_values[:, 1] == objectives[1].values)
    x_values = x.values
    assert np.all(
